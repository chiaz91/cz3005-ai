...
% To determine precedence, we first examine whether X and Y is in different gender group
% as male always having higher precedence than female
% however, within same gender group, age rule is applied
prior(X, Y):- prince(X),princess(Y).
prior(X, Y):- prince(X),prince(Y),older_than(X, Y).
prior(X, Y):- princess(X),princess(Y),older_than(X, Y).
